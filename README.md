# Personal Information Manager

## Project Description
This is my first Python project! It is a program that stores and displays personal information.



## What I Learned
- Variables: How to store different types of data
- Input/Output: Getting user input and displaying results
- String Formatting: Using f-strings to create clean output
- Error Handling: Basic validation for user input



## How to Run This Program

1. Make sure Python is installed
2. Open terminal or command prompt
3. Navigate to the project folder
4. Run the following command:
```bash
python personal_info.py

Features
Stores static information (name, age, city, hobby)
Gets user input (favorite food and favorite color)
Displays information in a formatted way
Basic input validation
Calculates age in months


Sample Output
========================================
      PERSONAL INFORMATION MANAGER
========================================

Name: Dnyaneshwari Shinde
Age: 20 years (240 months old)
City: Pune
Hobby: Reading Books

Favorite Food: Pizza
Favorite Color: Blue

========================================
Thank you for using the program!
========================================

Challenges & Solutions
Challenge:

User might enter empty input.

Solution:

Used a while loop for input validation.

Challenge:

Formatting the output properly.

Solution:

Used f-strings and separators for better readability.

Technologies Used
Python

Author
Dnyaneshwari Shinde




